...
- **Bazarr dual credential encoding (plaintext API vs hashed file):** the settings API accepts the auth password in plaintext while the stored `config.yaml` holds its lowercase MD5 hex — so the API path posts plaintext and only the YAML fallback (rewrite + service restart) hashes. Forcing string types on auth fields avoids the numeric-password rejection bug.
